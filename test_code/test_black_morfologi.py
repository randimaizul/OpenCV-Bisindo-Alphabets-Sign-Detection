...
# ================ Negative =============================
# Create negative setelah adaptive
imagem = cv2.bitwise_not(th3)
cv2.imwrite("train_data/negative.png", imagem)
cv2.imshow("negative",imagem)

#6 Dilasi
dilasi_negative = cv2.dilate(imagem,kernel,iterations = 1)
# cv2.imshow("Dilasi",dilasi)
cv2.imwrite("train_data/dilasi_negative.png", dilasi_negative)

#7 erosi
erosi_negative = cv2.erode(imagem,kernel,iterations = 1)
# cv2.imshow("Erosi",erosi)
cv2.imwrite("train_data/erosi_negative.png", erosi_negative)

#8 Closing
erosi_close = cv2.morphologyEx(imagem,cv2.MORPH_CLOSE, kernel)
# cv2.imshow("Morfo Closing",erosi_close)
cv2.imwrite("train_data/morfo_closing_3.png", erosi_close)

#9 opening
erosi_open = cv2.morphologyEx(imagem,cv2.MORPH_OPEN, kernel)
# cv2.imshow("Morfo Open",erosi_open)
cv2.imwrite("train_data/morfo_opening_3.png", erosi_open)
# ================ Negative =============================
...
plt.subplot(131),plt.imshow(th3,cmap = 'gray')
plt.title('ADAPTIVE_GAUSSIAN'), plt.xticks([]), plt.yticks([])

plt.subplot(132),plt.imshow(dilasi_negative,cmap = 'gray')
plt.title('dilasi'), plt.xticks([]), plt.yticks([])

plt.subplot(133),plt.imshow(erosi_negative,cmap = 'gray')
